...
volatile uint8_t i2c_write_complete = 1;
volatile uint8_t i2c_read_complete = 1;
...
void i2c_write(uint8_t address, uint8_t *p_data, uint8_t data_length)
{
    ...
    while(!i2c_write_complete);
    delay_ms(5); // Required for page write refresh
}
